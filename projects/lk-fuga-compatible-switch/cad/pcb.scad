...
difference() {
    translate([0,0,1.2]) insert_body($fn=30);

    // Front PCB
    translate([0,0,2]) linear_extrude(10) offset(r=-1) front_pcb_profile($fn=30);    
    ...
    *linear_extrude(3) intersection() {
        front_pcb_profile();
        offset(r=-1) back_profile();
    }
    
    
    // Assembly screws
    for(i=[1,-1]) for(ii=[-1,1]) translate([i*17,ii*15, -25]) linear_extrude(35) circle(d=3,$fn=30); 
    
    
}

*translate([0,0,1.2+1]) front_pcb($fn=30);
*translate([4.5,0,0]) rotate([-90,0,90]) back_pcb(parts=true, $fn=60);
...
module back_pcb(parts=false) {
    color("green") translate([-40/2, 0]) linear_extrude(1.6) back_pcb_profile();
    if(parts) {
