...
translate([120,0]) mirror([1,0,0]) idler_assembly($fn=60);
driver_assembly_test($fn=60);


module driver_assembly_test() {
    
    // assembly specific variables
    BRACKET_LENGTH =  MOTOR_POS_VECTOR[1] + NEMA17_SIDE_LENGTH/2;
    MOUNTING_SCREWS_FROM_BOTTOM = 15;
    ...
    difference() {
        ...
        // mounting screws
        translate([0,MOUNTING_SCREWS_FROM_TOP]) linear_extrude(BRACKET_BACK+BRACKET_THICKNESS) cleat_cutout_profile();
    ...
    translate([BRACKET_WIDTH-(BRACKET_WIDTH-NEMA17_SIDE_LENGTH), ROLLER_POS_VECTOR[1]]) linear_extrude(BRACKET_BACK+7) {
        ...
        translate([-ROLLER_COVER_THICKNESS,BRACKET_WIDTH-NEMA17_SIDE_LENGTH]) square([ROLLER_COVER_THICKNESS,PULLEY_CENTER_CENTER-27.5]);
        translate([-1,BRACKET_WIDTH-NEMA17_SIDE_LENGTH+PULLEY_CENTER_CENTER-27.5]) square([1,5]);
    }
    ...
    *translate([0,BRACKET_LENGTH]) color("yellow") pcb_mount();
    
    *translate(concat(MOTOR_POS_VECTOR, [BRACKET_BACK+BRACKET_THICKNESS])) rotate([180,0,0]) nema17(); 
}


module cleat_cutout_profile() {
    *polygon([
        [2,0], 
        [0,5], 
        [2+MOUNTING_SCREW_HEAD_HEIGHT,0], 
        [2+MOUNTING_SCREW_HEAD_HEIGHT, MOUNTING_SCREW_HEAD_DIAMETER+MOUNTING_SCREW_BODY_DIAMETER],
        [0,MOUNTING_SCREW_HEAD_DIAMETER+MOUNTING_SCREW_BODY_DIAMETER]
    ]);
    
    translate([0,MOUNTING_SCREW_BODY_DIAMETER]) square([2, MOUNTING_SCREW_HEAD_DIAMETER]);
    translate([2,0]) square([MOUNTING_SCREW_HEAD_HEIGHT, MOUNTING_SCREW_HEAD_DIAMETER+MOUNTING_SCREW_BODY_DIAMETER]);
}
...
module pcb_mount() {
    ...
    PCB_SIZE = [40, 60, 1.6];
    PCB_SLOT = [for(d = PCB_SIZE) d+0.4];
    PCB_CLEARANCES = [6, 23, 2]; // motor-pcb (under pcb), above pcb, maximum edge grap clearance;
    
    PCB_HOUSE_HEIGHT = PCB_CLEARANCES[0] + PCB_SLOT[2] + PCB_CLEARANCES[1];
    
    difference() {
        linear_extrude(PCB_SLOT[1]+PCB_CLEARANCES[2]+1) square([NEMA17_SIDE_LENGTH,PCB_HOUSE_HEIGHT+2]);
        linear_extrude(PCB_SLOT[1]+PCB_CLEARANCES[2]) translate([NEMA17_SIDE_LENGTH/2, PCB_CLEARANCES[0]]) square([PCB_SLOT[0], PCB_SLOT[2]], center=true);
        linear_extrude(PCB_SLOT[1]) translate([NEMA17_SIDE_LENGTH/2, PCB_HOUSE_HEIGHT/2]) square([PCB_SLOT[0]-PCB_CLEARANCES[2]*2, PCB_HOUSE_HEIGHT], center=true);
        
        #translate([NEMA17_SIDE_LENGTH/2,PCB_HOUSE_HEIGHT]) rotate([-90,0,0]) linear_extrude(2) scale([1.5,1,1]) circle(d=5);
    }
}
